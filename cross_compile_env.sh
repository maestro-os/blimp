...
export CC=clang
#export CFLAGS="-target i686-unknown-linux-musl --sysroot $TOOLCHAIN -L$TOOLCHAIN/usr/lib -L$TOOLCHAIN/lib -fuse-ld=lld --rtlib=compiler-rt --static"
export CFLAGS="-target i686-unknown-linux-musl --sysroot $TOOLCHAIN"

export LD=ld.lld
export LDFLAGS="-target i686-unknown-linux-musl --sysroot $TOOLCHAIN -L$TOOLCHAIN/usr/lib -L$TOOLCHAIN/lib -fuse-ld=lld --rtlib=compiler-rt --static"
